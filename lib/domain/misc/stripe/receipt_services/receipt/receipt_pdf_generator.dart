...
Future<Uint8List> generateReceiptPdf(
    ...
    UserProfileModel vendorUser,
    EventMerchantVendorProfile vendorProfile,
    AttendeeItem attendee,
    int? attendingNumber) async {
  
  final pdf = pw.Document();
  final fontSize = 9.0;
  final lineThickness = 1.0;
  final double columnWidth = PdfPageFormat.a4.width / 2 - 48;
  final greyColor = PdfColors.grey;
  final int invoiceNumber = attendingNumber ?? 1;

  // Load the custom font
  final fontData = await rootBundle.load("assets/fonts/Roboto-VariableFont_wdth.ttf");
  final font = pw.Font.ttf(fontData);

  final List<MVBoothPayments> successfulPayments = attendee.vendorForm?.boothPaymentOptions
          ?.where((e) => e.stripePaymentIntent?.status == 'succeeded')
          .toList() ?? [];
  ...
  final groupedPayments = groupBy(successfulPayments, (MVBoothPayments payment) => payment.uid);
  final groupedTaxTypes = groupBy(successfulPayments, (MVBoothPayments payment) => payment.stripePaymentIntent?.stripe_tax_detail?.taxType ?? 'HST');

  final Map<UniqueId, Map<String, dynamic>> allItemsByType = groupedPayments.map((uid, payments) {
    final fee = completeTotalPriceWithCurrency((payments.first.fee ?? 0).toDouble(), activity.rulesService.currency);
    final boothTitle = payments.first.boothTitle ?? 'Booth';
    ...
    pw.MultiPage(
      pageTheme: pw.PageTheme(
        ...
        buildBackground: (context) {
          return pw.FullPage(
            ignoreMargins: true,
            child: pw.Container(color: const PdfColor.fromInt(0xFFEAF5F6)),
          );
        },
      ),
      build: (pw.Context context) {
        return [
          pw.Column(
            ...
            children: [
              pw.Text('ACIRCLE', style: pw.TextStyle(font: font, fontSize: 20, fontWeight: pw.FontWeight.bold)),
              pw.Divider(thickness: lineThickness, color: greyColor),
              pw.SizedBox(height: 10),
              pw.Text('Thanks for joining, ${vendorUser.legalName.getOrCrash()}', style: pw.TextStyle(font: font, fontSize: 20)),
              ...
              pw.Divider(thickness: lineThickness, color: greyColor),
              pw.SizedBox(height: 10),
              pw.Text('Invoice number: ${getFirstSevenCharacters(attendee.attendeeId.getOrCrash())}-${invoiceNumber.toString().padLeft(4, '0')}', style: pw.TextStyle(font: font, fontSize: fontSize)),
              pw.Text('Receipt number: ${getFirstSevenCharacters(attendee.attendeeId.getOrCrash())}', style: pw.TextStyle(font: font, fontSize: fontSize)),
              pw.Text('PDF Created: ${DateFormat.yMMMMd().format(DateTime.now())}', style: pw.TextStyle(font: font, fontSize: fontSize)),
              pw.SizedBox(height: 20),

              // Summary Table
              pw.Column(
                children: [
                  buildTableRow(description: '', quantity: 'Qty', unitPrice: 'Price', amount: 'Amount', fontSize: fontSize, font: font),
                  pw.Divider(thickness: lineThickness, color: greyColor),
              ...
              // Payments Section
              pw.Text('Payments', style: pw.TextStyle(font: font, fontSize: fontSize, fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 5),
              pw.Divider(thickness: lineThickness, color: greyColor),
                ...
                children: [
                  pw.Container(
                    ...
                    child: pw.Column(
                      ...
                      children: [
                        pw.Divider(thickness: lineThickness, color: greyColor),
                        buildSummaryRow('Subtotal', itemsTotalFormatted, fontSize, font: font),
                        buildSummaryRow('Service Fee', totalBuyerFeeFormatted, fontSize, font: font),
                        pw.Divider(thickness: lineThickness, color: greyColor),
                        buildSummaryRow('Total', totalFormatted, fontSize, isBold: true, font: font),
                      ],
                    ),
                  ),
                ],
